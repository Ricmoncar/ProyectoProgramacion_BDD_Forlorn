      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          if (razaId === undefined || razaId === null) {
            console.error(
              "ID de raza no encontrado en los datos de la fila:",
              row
            );
            return "Error ID";
          }
...
function verRaza(id) {
  ...
  fetch(`/obtener_raza?id=${id}`)
    ...
    .then((raza) => {
      ...
      const alturaTexto =
        raza.alturaPromedia !== null
          ? `${raza.alturaPromedia.toFixed(2)} m`
          : "No especificada";
      const anchoTexto =
        raza.anchoPromedio !== null
          ? `${raza.anchoPromedio.toFixed(2)} m`
          : "No especificado";
...
function editarRaza(id) {
  ...
  fetch(`/obtener_raza?id=${id}`)
    ...
    .then((raza) => {
      ...
      document.getElementById("racePhysicalDesc").value =
        raza.descripcionFisica || "";
    ...
    .catch((error) => {
      ...
      mostrarAlerta(
        "error",
        `Error al cargar datos de la raza: ${error.message}`
      );
      cerrarModal();
    });
}
...
function eliminarRaza(id) {
  ...
  if (
    confirm(
      `¿Está seguro que desea eliminar la raza con ID ${id}? Esta acción no se puede deshacer.`
    )
  ) {
    fetch(`/eliminar_raza?id=${id}`)
      ...
      .then((resultado) => {
        mostrarAlerta(
          "success",
          `Raza ${id} eliminada correctamente. ${resultado}`
        );
        cargarRazas();
      })
      .catch((error) => {
...
function guardarRaza() {
  ...
  const descripcionFisica = document
    .getElementById("racePhysicalDesc")
    .value.trim();
  ...
  fetch(url)
    ...
    .then((resultado) => {
      mostrarAlerta(
        "success",
        id ? "Raza actualizada correctamente." : "Raza añadida correctamente."
      );
      cerrarModal();
      cargarRazas();
    })
...
function aplicarFiltros() {
  ...
  if (alturaMinima && parseFloat(alturaMinima) > 0)
    params.append("alturaMinima", alturaMinima);
  if (anchoMinimo && parseFloat(anchoMinimo) > 0)
    params.append("anchoMinimo", anchoMinimo);
