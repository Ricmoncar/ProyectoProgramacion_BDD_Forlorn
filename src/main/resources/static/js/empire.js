  ...
  fetch("/listar_imperios")
    ...
    .then((imperios) => {
      ...
      if (Array.isArray(imperios)) {
        ...
        console.error(
          "La respuesta de listar_imperios no es un array:",
          imperios
        );
        mostrarAlerta(
          "error",
          "Error: Formato de datos de imperios inesperado."
        );
      }
    })
    .catch((error) => {
...
function verImperio(id) {
  ...
  fetch(`/obtener_imperio?id=${id}`)
    ...
    .then((imperio) => {
      ...
      const poblacionTexto = imperio.poblacion
        ? imperio.poblacion.toLocaleString("es-ES") + " habitantes"
        : "No especificada";
      const gdpTexto = imperio.gdp
        ? imperio.gdp.toLocaleString("es-ES") + " millones"
        : "No especificado";
      const tamanioTexto = imperio.tamanio
        ? imperio.tamanio.toLocaleString("es-ES") + " km²"
        : "No especificado";
      ...
      const detallesHTML = `
                <div class="empire-details">
                    ...
                    <div class="detail-row">
                        ...
                        <span class="detail-value">${
                          imperio.ideologia || "No especificada"
                        }</span>
                    </div>
                    <div class="detail-row">
...
function editarImperio(id) {
  ...
  fetch(`/obtener_imperio?id=${id}`)
    ...
    .then((imperio) => {
      ...
      document.getElementById("empirePopulation").value =
        imperio.poblacion || "";
      document.getElementById("empireIdeology").value =
        imperio.ideologia || "Monarquía";
      document.getElementById("empireGDP").value = imperio.gdp || "";
      document.getElementById("empireSize").value = imperio.tamanio || "";
      document.getElementById("empireDescription").value =
        imperio.descripcion || "";
      document.getElementById("empireAtWar").value = imperio.enGuerra
        ? "true"
        : "false";
    ...
    .catch((error) => {
      ...
      mostrarAlerta(
        "error",
        `Error al cargar datos del imperio: ${error.message}`
      );
      cerrarModal();
    });
}
...
function eliminarImperio(id) {
  ...
  if (
    confirm(
      `¿Está seguro que desea eliminar el imperio con ID ${id}? Esta acción no se puede deshacer.`
    )
  ) {
    fetch(`/eliminar_imperio?id=${id}`)
      ...
      .then((resultado) => {
        mostrarAlerta(
          "success",
          `Imperio ${id} eliminado correctamente. ${resultado}`
        );
        cargarImperios();
      })
      .catch((error) => {
  ...
  fetch(url)
    ...
    .then((resultado) => {
      mostrarAlerta(
        "success",
        id
          ? "Imperio actualizado correctamente."
          : "Imperio añadido correctamente."
      );
      cerrarModal();
      cargarImperios();
    })
...
function aplicarFiltros() {
  ...
  const enGuerra =
    enGuerraSelect && enGuerraSelect.value
      ? enGuerraSelect.value === "true"
      : null;
  ...
  if (poblacionMinima && parseInt(poblacionMinima) > 0)
    params.append("poblacionMinima", poblacionMinima);
  ...
  fetch(url)
    ...
    .then((imperios) => {
      ...
      if (Array.isArray(imperios)) {
        ...
        console.error(
          "La respuesta de filtrar_imperios no es un array:",
          imperios
        );
        mostrarAlerta(
          "error",
      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          if (imperioId === undefined || imperioId === null) {
            console.error(
              "ID de imperio no encontrado en los datos de la fila:",
              row
            );
            return "Error ID";
          }
