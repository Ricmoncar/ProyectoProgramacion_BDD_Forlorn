  ...
  fetch("/listar_razas")
    ...
    .then((razas) => {
      ...
      if (!selectRaza || !filterRaza) {
        console.error(
          "No se encontraron los elementos select de raza en el DOM"
        );
        return;
      }
    ...
    .catch((error) => {
      ...
      mostrarAlerta(
        "error",
        `No se pudieron cargar las razas: ${error.message}`
      );
    });
}
  ...
  fetch("/listar_imperios")
    ...
    .then((imperios) => {
      ...
      if (!selectImperio || !filterImperio) {
        console.error(
          "No se encontraron los elementos select de imperio en el DOM"
        );
        return;
      }
    ...
    .then((personas) => {
      ...
      if (Array.isArray(personas)) {
        ...
        personas.forEach((persona) => {
          ...
          persona.imperioNombre = persona.imperio
            ? persona.imperio.nombre
            : "N/A";
        });
        ...
        console.error(
          "La respuesta de listar_personas no es un array:",
          personas
        );
        mostrarAlerta(
          "error",
          "Error: Formato de datos de personas inesperado."
        );
      }
    })
    .catch((error) => {
  ...
  fetch(`/obtener_persona?id=${id}`)
    ...
    .then((persona) => {
                        ...
                        <div class="detail-grid">
                            <div class="detail-item">
                                ...
                                <span class="detail-value">${
                                  persona.raza ? persona.raza.nombre : "N/A"
                                }</span>
                            </div>
                            <div class="detail-item">
                            ...
                            <div class="detail-item">
                                ...
                                <span class="detail-value">${
                                  persona.oro
                                    ? persona.oro.toLocaleString("es-ES")
                                    : "0"
                                }</span>
                            </div>
                            <div class="detail-item">
                    ...
                    <div class="person-section">
                        ...
                        <div class="detail-grid">
                            <div class="detail-item">
                                ...
                                <span class="detail-value">${
                                  persona.alto ? persona.alto + " m" : "N/A"
                                }</span>
                            </div>
                            <div class="detail-item">
                                <span class="detail-label">Ancho:</span>
                                <span class="detail-value">${
                                  persona.ancho ? persona.ancho + " m" : "N/A"
                                }</span>
                            </div>
                            <div class="detail-item">
                                <span class="detail-label">% Grasa:</span>
                                <span class="detail-value">${
                                  persona.porcentajeGrasaCorporal
                                    ? persona.porcentajeGrasaCorporal + "%"
                                    : "N/A"
                                }</span>
                            </div>
        ...
        detallesHTML += `
                    <div class="person-section">
                        ...
                        <div class="stats-display">
                            <div class="stat-item">ATK: <span>${
                              stats.atk || 0
                            }</span></div>
                            <div class="stat-item">DEF: <span>${
                              stats.def || 0
                            }</span></div>
                            <div class="stat-item">HP: <span>${
                              stats.hp || 0
                            }</span></div>
                            <div class="stat-item">SPE: <span>${
                              stats.spe || 0
                            }</span></div>
                            <div class="stat-item">MAT: <span>${
                              stats.mat || 0
                            }</span></div>
                            <div class="stat-item">MDF: <span>${
                              stats.mdf || 0
                            }</span></div>
                            <div class="stat-item">XP: <span>${
                              stats.xp || 0
                            }</span></div>
                            <div class="stat-item">LVL: <span>${
                              stats.lvl || 1
                            }</span></div>
                        </div>
                    </div>
      ...
      cargarEquipamientoPersona(persona.id, function (equipamiento) {
        if (
          equipamiento &&
          (equipamiento.armas.length > 0 ||
            equipamiento.armaduras.length > 0 ||
            equipamiento.herramientas.length > 0 ||
            equipamiento.arcanas.length > 0)
        ) {
          ...
          if (equipamiento.armas.length > 0) {
            ...
            equipamiento.armas.forEach((arma) => {
              equipamientoHTML += `<li><span class="equipment-name">${
                arma.nombre
              }</span> <span class="equipment-property">${
                arma.material || "Material desconocido"
              }</span></li>`;
            });
          ...
          if (equipamiento.armaduras.length > 0) {
            ...
            equipamiento.armaduras.forEach((armadura) => {
              equipamientoHTML += `<li><span class="equipment-name">${
                ...
                armadura.material || "Material desconocido"
              }</span></li>`;
            });
            equipamientoHTML += `</ul></div>`;
          }
          ...
          if (equipamiento.arcanas.length > 0) {
            ...
            equipamiento.arcanas.forEach((arcana) => {
              equipamientoHTML += `<li><span class="equipment-name">${
                arcana.tipo
              }</span> <span class="equipment-property">Maestría: ${
                arcana.maestria || "Básica"
              }</span></li>`;
            });
            equipamientoHTML += `</ul></div>`;
          }
...
function editarPersona(id) {
  ...
  fetch(`/obtener_persona?id=${id}`)
    ...
    .then((persona) => {
      ...
      document.getElementById("personBodyFat").value =
        persona.porcentajeGrasaCorporal || "";
      document.getElementById("personPhysicalDesc").value =
        persona.descripcionFisica || "";
      document.getElementById("personPersonality").value =
    ...
    .catch((error) => {
      ...
      mostrarAlerta(
        "error",
        `Error al cargar datos de la persona: ${error.message}`
      );
      cerrarModal();
    });
}
...
function eliminarPersona(id) {
  ...
  if (
    confirm(
      `¿Está seguro que desea eliminar la persona con ID ${id}? Esta acción no se puede deshacer.`
    )
  ) {
...
function guardarPersona() {
  ...
  const personaData = {
    ...
    razaId: document.getElementById("personRace").value
      ? parseInt(document.getElementById("personRace").value)
      : null,
    imperioId: document.getElementById("personEmpire").value
      ? parseInt(document.getElementById("personEmpire").value)
      : null,
    ...
    porcentajeGrasaCorporal:
      parseFloat(document.getElementById("personBodyFat").value) || null,
    descripcionFisica:
      document.getElementById("personPhysicalDesc").value.trim() || null,
    personalidad:
      document.getElementById("personPersonality").value.trim() || null,
  ...
  document
    .querySelectorAll("#personWeapons .equipment-item-selected")
    .forEach((item) => {
  ...
  document
    .querySelectorAll("#personArmor .equipment-item-selected")
    .forEach((item) => {
      const equipmentId = item.getAttribute("data-equipment-id");
      if (equipmentId) {
  ...
  document
    .querySelectorAll("#personTools .equipment-item-selected")
    .forEach((item) => {
      const equipmentId = item.getAttribute("data-equipment-id");
      if (equipmentId) {
  ...
  document
    .querySelectorAll("#personArcana .equipment-item-selected")
    .forEach((item) => {
      const equipmentId = item.getAttribute("data-equipment-id");
      if (equipmentId) {
  ...
  fetch(url)
    ...
    .then((personas) => {
      ...
      if (Array.isArray(personas)) {
        ...
        console.error(
          "La respuesta de filtrar_personas no es un array:",
          personas
        );
        mostrarAlerta(
          "error",
  ...
  fetch(endpoint)
    ...
    .then((items) => {
      ...
      items.forEach((item) => {
        ...
        const description =
          item.descripcion || item.material || "Sin descripción";
...
function selectEquipment(equipmentId, equipmentType, equipmentName) {
  ...
  if (!personId) {
    mostrarAlerta(
      "warning",
      "Debe guardar la persona primero antes de añadir equipamiento"
    );
    return;
  }
