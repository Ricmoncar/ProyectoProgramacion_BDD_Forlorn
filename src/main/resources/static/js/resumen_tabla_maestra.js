...
function inicializarTablaMaestra() {
    let tablaMaestraDT = $('#tablaMaestra').DataTable({
        ...
        lengthMenu: [[10, 25, 50, -1], [10, 25, 50, "Todos"]], // Opciones para mostrar registros
        columns: [
            ...
            { data: 'infoClave1', title: 'Info Clave 1', render: function(data){ return data || 'N/A';}},
            { data: 'infoClave2', title: 'Info Clave 2', render: function(data){ return data || 'N/A';}},
            {
    ...
    switch (data.tipoEntidad) {
        case 'Planeta':     borderColor = '#2980b9'; break; // Azul
        case 'Imperio':     borderColor = '#8e44ad'; break; // Púrpura
        case 'Guerra':      borderColor = 'var(--crimson, #DC143C)'; break; // Carmesí
        case 'Persona':     borderColor = '#27ae60'; break; // Verde
        case 'Raza':        borderColor = '#f39c12'; break; // Naranja
        case 'Continente':  borderColor = '#16a085'; break; // Turquesa
        case 'Bioma':       borderColor = '#d35400'; break; // Naranja oscuro
        ...
        case 'Arcana':      borderColor = '#7f8c8d'; break; // Gris para equipamiento
    }
        ...
        .then(response => {
            if (!response.ok) {
                return response.text().then(text => {
                    throw new Error(`Error HTTP ${response.status} (${response.statusText}) al cargar tabla maestra. Detalle: ${text}`);
                });
            }
        ...
        .then(datos => {
            if (Array.isArray(datos)) {
                ...
                console.error("Formato de datos inesperado para tabla maestra:", datos);
                 ...
                 $('#tablaMaestra tbody').html('<tr><td colspan="7">Error: Formato de datos incorrecto.</td></tr>');
            }
        })
        .catch(error => {
            console.error('Error al cargar datos para tabla maestra:', error);
            $('#tablaMaestra tbody').html(`<tr><td colspan="7">Error al cargar datos: ${error.message}</td></tr>`);
        });
}