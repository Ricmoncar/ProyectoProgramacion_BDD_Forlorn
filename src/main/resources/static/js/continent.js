      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          if (continentId === undefined || continentId === null) {
            console.error(
              "ID de continente no encontrado en los datos de la fila:",
              row
            );
            return "Error ID";
          }
  ...
  fetch("/listar_continentes")
    ...
    .then((continentes) => {
      ...
      if (Array.isArray(continentes)) {
        ...
        console.error(
          "La respuesta de listar_continentes no es un array:",
          continentes
        );
        mostrarAlerta(
          "error",
          "Error: Formato de datos de continentes inesperado."
        );
      }
    })
    .catch((error) => {
      ...
      mostrarAlerta(
        "error",
        `No se pudieron cargar los continentes: ${error.message}`
      );
    });
}
...
function verContinente(id) {
  ...
  fetch(`/obtener_continente?id=${id}`)
    ...
    .then((continente) => {
      ...
      const esHabitable =
        continente.habitable == 1 || continente.habitable === true;
      const tamanioTexto = continente.tamanio
        ? `${continente.tamanio} km²`
        : "No especificado";
      const descripcionTexto =
        continente.descripcion || "No hay descripción disponible.";
                ...
                <div class="continent-details">
                    <div class="detail-row">
                        ...
                        <span class="detail-value">${
                          continente.planetaNombre || "N/A"
                        }</span>
                    </div>
                    <div class="detail-row">
                        <span class="detail-label">Hemisferio:</span>
                        <span class="detail-value">${
                          continente.hemisferio || "N/A"
                        }</span>
                    </div>
                    <div class="detail-row">
                        <span class="detail-label">Clima:</span>
                        <span class="detail-value">${
                          continente.clima || "N/A"
                        }</span>
                    </div>
                    <div class="detail-row">
    ...
    .catch((error) => {
      ...
      mostrarAlerta(
        "error",
        `Error al obtener el continente: ${error.message}`
      );
    });
}
...
function editarContinente(id) {
  ...
  fetch(`/obtener_continente?id=${id}`)
    ...
    .then((continente) => {
      if (!continente) {
        throw new Error(
          "No se recibió información del continente para editar."
        );
      }

      document.getElementById("continentName").value = continente.nombre || "";
      document.getElementById("continentPlanet").value =
        continente.planetaId || "";
      document.getElementById("continentHemisphere").value =
        continente.hemisferio || "Norte";
      document.getElementById("continentClimate").value =
        continente.clima || "Templado";
      ...
      document.getElementById("continentDescription").value =
    ...
    .catch((error) => {
      ...
      mostrarAlerta(
        "error",
        `Error al cargar datos del continente: ${error.message}`
      );
      cerrarModal();
    });
}
...
function eliminarContinente(id) {
  ...
  if (
    confirm(
      `¿Está seguro que desea eliminar el continente con ID ${id}? Esta acción no se puede deshacer.`
    )
  ) {
    fetch(`/eliminar_continente?id=${id}`)
      ...
      .then((resultado) => {
        mostrarAlerta(
          "success",
          `Continente ${id} eliminado correctamente. ${resultado}`
        );
        cargarContinentes();
      })
      .catch((error) => {
        ...
        mostrarAlerta(
          "error",
          `Error al eliminar continente: ${error.message}`
        );
      });
  }
}
...
function guardarContinente() {
  ...
  const descripcion = document
    .getElementById("continentDescription")
    .value.trim();
    ...
    .then((resultado) => {
      mostrarAlerta(
        ...
        id
          ...
          : "Continente añadido correctamente."
      );
      cerrarModal();
      cargarContinentes();
    })
...
function aplicarFiltros() {
  ...
  const habitable =
    habitableSelect && habitableSelect.value
      ? habitableSelect.value === "true"
      : null;
  ...
  fetch(url)
    ...
    .then((continentes) => {
      ...
      if (Array.isArray(continentes)) {
        ...
        console.error(
          "La respuesta de filtrar_continentes no es un array:",
          continentes
        );
        mostrarAlerta(
          "error",
