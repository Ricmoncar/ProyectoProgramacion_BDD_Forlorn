      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          if (biomaId === undefined || biomaId === null) {
            console.error(
              "ID de bioma no encontrado en los datos de la fila:",
              row
            );
            return "Error ID";
          }
  ...
  fetch("/listar_continentes")
    ...
    .then((continentes) => {
      ...
      if (!selectContinente || !filterContinente) {
        console.error(
          "Elementos select de continente no encontrados en el DOM."
        );
        return;
      }
...
function cargarBiomas() {
  fetch("/listar_biomas")
    ...
    .catch((error) => {
      ...
      mostrarAlerta(
        "error",
        `No se pudieron cargar los biomas: ${error.message}`
      );
    });
}
...
function verBioma(id) {
  ...
  fetch(`/obtener_bioma?id=${id}`)
    ...
    .then((bioma) => {
      ...
      const porcentajeHumedadTexto =
        bioma.porcentajeHumedad !== null
          ? `${bioma.porcentajeHumedad}%`
          : "No especificado";
      const temperaturaMediaTexto =
        bioma.temperaturaMedia !== null
          ? `${bioma.temperaturaMedia}°C`
          : "No especificada";
...
function editarBioma(id) {
  ...
  fetch(`/obtener_bioma?id=${id}`)
    ...
    .then((bioma) => {
      ...
      document.getElementById("biomeContinent").value =
        bioma.continenteId || "";
      ...
      if (humedadInput) {
        humedadInput.value =
          bioma.porcentajeHumedad !== null ? bioma.porcentajeHumedad : "";
      }

      const precipitacionesSelect =
        document.getElementById("biomePrecipitation");
      ...
      if (tempInput) {
        tempInput.value =
          bioma.temperaturaMedia !== null ? bioma.temperaturaMedia : "";
      }
    ...
    .catch((error) => {
      ...
      mostrarAlerta(
        "error",
        `Error al cargar datos del bioma: ${error.message}`
      );
      cerrarModal();
    });
}
...
function eliminarBioma(id) {
  ...
  if (
    confirm(
      `¿Está seguro que desea eliminar el bioma con ID ${id}? Esta acción no se puede deshacer.`
    )
  ) {
    fetch(`/eliminar_bioma?id=${id}`)
      ...
      .then((resultado) => {
        mostrarAlerta(
          "success",
          `Bioma ${id} eliminado correctamente. ${resultado}`
        );
        cargarBiomas();
      })
      .catch((error) => {
...
function guardarBioma() {
  ...
  fetch(url)
    ...
    .then((resultado) => {
      mostrarAlerta(
        "success",
        id ? "Bioma actualizado correctamente." : "Bioma añadido correctamente."
      );
      cerrarModal();
      cargarBiomas();
    })
...
function aplicarFiltros() {
  ...
  if (humedadMinima && parseInt(humedadMinima) > 0)
    params.append("humedadMinima", humedadMinima);
